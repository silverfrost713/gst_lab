...
struct AESMatrix {
	/* Ìàòðèöà ïðåäñòàâëåíà ìàññèâîì áàéò. */
	aesbyte_t* grid;
	aesbyte_t side_len;
};

/* Ôóíêöèÿ èíèöèàëèçàöèè ìàòðèöû. */
void aes_matrix_init(struct AESMatrix* m, aesbyte_t side_len) {
	/* Ïðîâåðèòü äëèíó ñòîðîíû íà êîððåêòíîñòü. */
	if (!side_len) {
		fprintf(stderr, "ERROR in aes_matrix_init(): wrong side length specified \"%d\".\n", side_len);
		return;
	}
	/* Çàäàòü äëèíó ñòîðîíû ìàòðèöû, âûäåëèòü ïàìÿòü è ïðîâåðèòü óêàçàòåëü. */
	m->side_len = side_len;
...
void aes_matrix_free(struct AESMatrix* m) {
	...
	/* Îñâîáîäèòü è óêàçàòåëü âíóòðè ìàòðèöû, è óêàçàòåëü íà ñàìó ìàòðèöó. */
	else
		free(m->grid);
}

/* Ôóíêöèÿ âûâîäà ìàòðèöû â ñòàíäàðòíûé âûâîä. */
void aes_matrix_display(struct AESMatrix m) {
	...
	/* Ïðîâåðèòü äëèíó ñòîðîíû ìàòðèöû. */
	if (!m.side_len) {
		fprintf(stderr, "ERROR in aes_matrix_display(): got matrix with null size.\n");
		return;
	}
	/* Âûâåñòè ìàòðèöó ïîñòðî÷íî â ñòàíäàðòíûé âûâîä. */
	for (aesbyte_t i = 0; i < m.side_len; i += 1) {
		for (aesbyte_t j = 0; j < m.side_len; j += 1)
			/* Íå ïëîäèòü ëèøíèõ ïðîáåëîâ íà ñëó÷àé, åñëè âûâîä íàïðàâëåí â ôàéë. */
			if (j != m.side_len - 1)
...
/* Ôóíêöèÿ çàïîëíåíèÿ ìàòðèöû ñëó÷àéíûìè áàéòàìè. */
void aes_matrix_randomise(struct AESMatrix m) {
	...
	/* Çàïîëíèòü ìàòðèöó ñëó÷àéíûìè áàéòàìè. */
	for (aesbyte_t i = 0; i < m.side_len * m.side_len; i += 1)
		m.grid[i] = (aesbyte_t)rand();
}

/* Ôóíêöèÿ îïåðàöèè ShiftRows èç AES. */
void aes_matrix_shift_rows(struct AESMatrix m) {
	...
	/* Ñäâèãàòü ìàòðèöó âëåâî ïîñòðî÷íî, íà ÷èñëî ýëåìåíòîâ,
	   ðàâíîå èíäåêñó ñòðîêè, íà÷èíàÿ ñ íóëÿ. */
...
int tcp_establish_conn_client(SOCKET* connect_socket) {
	/* Ñåðâåð ïîêà íà localhost. */
	const char* server_addr = "127.0.0.1";
	/* WSADATA ñîäåðæèò ñâåäåíèÿ î ðåàëèçàöèè ñîêåòîâ â Windows. */
	WSADATA wsa_data;
	/* Ñòðóêòóðû addrinfo ñîäåðæàò äàííûå îá àäðåñàõ è ïðîòîêîëàõ. */
	struct addrinfo* result = NULL;
	struct addrinfo hints;
	int my_err;
	/* WSAStartup çàïðàøèâàåò âåðñèþ WinSocket 2.2 è íå äà¸ò êëèåíòàì
	   èñïîëüçîâàòü âåðñèþ âûøå. */
	...
	/* Çàäàòü ñîåäèíåíèå ïî ïðîòîêîëó TCP ÷åðåç ñîêåò. */
	memset(&hints, 0, sizeof(hints));
	hints.ai_family = AF_UNSPEC;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;
	/* Ðàçðåøèòü àäðåñ ñåðâåðà ñ ïîðòîì. */
	...
	*connect_socket = INVALID_SOCKET;
	/* Ïîïûòàòüñÿ ïîäêëþ÷èòüñÿ ïî àäðåñó, ïîëó÷åííîìó ÷åðåç getaddrinfo, ñîçäàòü ñîêåò. */
...
char* tcp_receive_matrices(size_t* recv_datasize, SOCKET connect_socket) {
	/* Áóôåðû äëÿ ïðè¸ìà ìåòàèíôîðìàöèè î ðàçìåðå äàííûõ,
	   à òàêæå äëÿ ñàìèõ ìàòðèö. */
	char tcp_datasize_recvbuf[DEF_BUFLEN];
	char* tcp_aes_data_recvbuf;
	size_t tcp_datasize;
	int recv_res;
	/* Ïðèíÿòü ìåòàèíôîðìàöèþ î ðàçìåðå ïåðåñûëàåìûõ äàííûõ ñ ìàòðèöàìè è âûäåëèòü ïàìÿòü. */
...
struct AESMatrix* aes_unflatten_matrices(char* aes_matrices_flat, size_t aes_datasize, size_t* matrix_qty) {
	...
	*matrix_qty = aes_matrix_qty;
	/* Ðàññ÷èòàòü ÷èñëî ìàòðèö, â êîòîðîå ðàñïàêóåòñÿ áóôåð. */
...
struct AESMatrix* generate_matrices(size_t gen_data_size, aesbyte_t gen_matrix_side, size_t* gen_len, size_t* gen_matrix_size) {
	*gen_matrix_size = ((size_t)gen_matrix_side * (size_t)gen_matrix_side + 1);
	...
	/* Ðàññ÷èòàòü êîëè÷åñòâî ìàòðèö äëÿ ãåíåðàöèè. */
	*gen_len = (gen_data_size * MEGABYTE_SIZE) / *gen_matrix_size;
	...
	/* Ïîëó÷èòü òåêóùåå âðåìÿ, ïðîâåðèòü åãî êîððåêòíîñòü è çàäàòü ñèäîì ÃÑ×. */
	time_t rand_seed = time(NULL);
	if (rand_seed == -1)
		fprintf(stderr, "WARNING in generate_matrices(): couldn't get current time for random seed.\n");
	srand((unsigned int)rand_seed);
	/* Èíèöèàëèçèðîâàòü è ãåíåðèðîâàòü ìàòðèöû AES èç ñëó÷àéíûõ áàéò. */
...
char* aes_pack_matrices(struct AESMatrix* gen_matrices, size_t gen_len, size_t gen_matrix_size) {
	...
	/* Óïàêîâàòü ìàòðèöó â ìàññèâ ÷àðîâ äëÿ ïåðåäà÷è ïî TCP. */
	for (size_t i = 0; i < gen_len; i += 1) {
...
int tcp_establish_conn_server(SOCKET* cl_socket) {
	...
	/* Ðàçðåøèòü ñîáñòâåííûé àäðåñ è ïîðò ñåðâåðà. */
	...
	/* Ñîçäàòü ñîêåò äëÿ ïðîñëóøèâàíèÿ. */
	...
	freeaddrinfo(result);
	/* Æäàòü, ïîêà íå ïîäêëþ÷èòñÿ êëèåíò. */
	...
	/* Ïðèíÿòü ïîäêëþ÷åíèå êëèåíòà ïðè åãî ïîÿâëåíèè. */
...
int tcp_send_matrices(char* aes_data, size_t aes_datasize, SOCKET client_socket) {
	...
	/* Îòïðàâèòü êëèåíòó ìåòàèíôîðìàöèþ î ðàçìåðå ïåðåñûëàåìûõ äàííûõ ñ ìàòðèöàìè. */
...
int tcp_send_time(int aes_time, SOCKET client_socket) {
	/* Áóôåð äëÿ îòïðàâêè èíôîðìàöèè î âðåìåíè âû÷èñëåíèé. */
	...
	/* Îòïðàâèòü êëèåíòó ìåòàèíôîðìàöèþ î âðåìåíè âû÷èñëåíèé. */
...
int tcp_recv_time(SOCKET connect_socket) {
	...
	int recv_res;
	int tcp_time;
	/* Ïðèíÿòü ìåòàèíôîðìàöèþ î âðåìåíè âû÷èñëåíèé. */
